...
> **Ветка `tls`.** Наружу выходит nginx с TLS, Grafana за ним порт не публикует. Логи контейнеров собираются чтением файлов. Сравнение веток — в разделе [«Четыре ветки»](#четыре-ветки).
> 
> **Есть четвертая реализация.** В ветке [`hardening`](../../tree/hardening) проработана безопасность стека, ограничен ущерб от возможной компрометации контейнера.
...
| [`tls`](../../tree/tls) | + nginx с TLS, правило на истечение сертификата, systemd | доступ по HTTPS без туннеля; сертификат самоподписанный |
| [`hardening`](../../tree/hardening) | + правила на ограничения capabilities, docker.socket | каждый слой уменьшает вероятность пробоя или урон от него |
...
[`tls...hardening`](../../compare/tls...hardening).
